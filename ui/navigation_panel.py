...
class NavigationPanel(QWidget):
    ...
    def __init__(self,db,parent=None):
        super().__init__(parent); self.db=db; self.history=[]; self.selected_book=None; self.setMinimumWidth(300); self.setMaximumWidth(360); self.setFixedWidth(330); self._init_ui()
    def _init_ui(self):
    ...
    def _on_book_clicked(self,item): book=item.data(Qt.ItemDataRole.UserRole); self._set_selected_book(book); self.book_selected.emit(book,self.chapter_spin.value())
    ...
    def _on_chapter_changed(self,chapter):
        if self.selected_book:self._set_verse_ranges(self.selected_book,chapter)
    ...
    def load_history(self,history_list):
        ...
        for entry in history_list or []:
            try:
                if len(entry)==4:self.history.append((entry[0],int(entry[1]),entry[2],entry[3]))
            except:pass
    ...
    def _update_history_list(self):
        ...
        for i,(book,chapter,start,end) in enumerate(self.history):
            text=f"{book}{chapter}章" if start is None else (f"{book}{chapter}章{start}节-本章末" if end is None else (f"{book}{chapter}章{start}节" if start==end else f"{book}{chapter}章{start}-{end}节")); item=QListWidgetItem(); self.history_list.addItem(item); widget=HistoryItemWidget(i,text); widget.clicked.connect(self._on_history_clicked); widget.deleted.connect(self._delete_history); item.setSizeHint(widget.sizeHint()); self.history_list.setItemWidget(item,widget)
    def _on_history_clicked(self,index):
        if not 0<=index<len(self.history):return
        book,chapter,start,end=self.history[index]; self.selected_book=book; self.chapter_spin.setRange(1,max(1,self.db.get_chapter_count(book))); self.chapter_spin.blockSignals(True); self.chapter_spin.setValue(chapter); self.chapter_spin.blockSignals(False); max_v=max(1,self.db.get_verse_count(book,chapter)); self.start_spin.setRange(1,max_v); self.end_spin.setRange(1,max_v); self.start_spin.setValue(max(1,min(start or 1,max_v))); self.end_spin.setValue(max(1,min(end or max_v,max_v))); self.range_selected.emit(book,chapter,start or 1,end or max_v); self.history.insert(0,self.history.pop(index)); self._update_history_list(); self.history_changed.emit(self.history)
