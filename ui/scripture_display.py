...
class ScriptureDisplay(QWidget):
    ...
    def _set_adaptive_title(self, text):
        ...
        # 标题栏高度随最终字号动态变化，避免上下裁切。
        title_height=max(52,QFontMetrics(font).height()+18)
    ...
    def _render_scripture(self):
        # 顶部留白至少覆盖经文字号的高度，底部给底注和下边缘留出安全区，避免上下被裁切。
        top_pad=max(14,int(self.font_size*0.65)); bottom_pad=self.footer_height+max(24,int(self.font_size*0.8))
