...
class ScriptureDisplay(QWidget):
    ...
    def set_reference_size(self,width,height):
        ...
        self._fit_document_width(); self.update()
    def clear_reference_size(self): self._reference_size=None; self._fit_document_width(); self.update()
    ...
    def _fit_document_width(self):
        ...
        if self._reference_size:
            actual_widget_width=max(1,self.width()); left_ratio=self.margin_left/max(1,actual_widget_width); right_ratio=self.margin_right/max(1,actual_widget_width); ref_width=max(1,self._reference_size[0]); content_width=max(1,ref_width-int(ref_width*(left_ratio+right_ratio))); self.text_display.document().setTextWidth(content_width)
        else:self.text_display.document().setTextWidth(actual_width)
    ...
    def _render_scripture(self):
        ...
        if self.verse_segmentation:html += "".join(f"<p style='margin:0 0 8px 0;padding:0;'>{self._verse_html(n,t)}</p>" for n,t in self.verses)
        else:html += "<p style='margin:0;padding:0;white-space:normal;'>"+" ".join(self._verse_html(n,t) for n,t in self.verses)+"</p>"
