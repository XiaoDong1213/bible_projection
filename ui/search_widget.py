...
class SearchWidget(QWidget):
    ...
    def __init__(self,db,parent=None):
        ...
        self.hint_label=QLabel("输入简拼　↑↓选择　空格选择/下一段　Enter确认　Esc退出"); self.hint_label.setMinimumHeight(32); self.hint_label.setAlignment(Qt.AlignmentFlag.AlignCenter); self.hint_label.setWordWrap(True); self.hint_label.setStyleSheet("QLabel { padding: 6px 10px; border-radius: 5px; background: rgba(0,0,0,35); }"); lay.addWidget(self.hint_label)
    ...
    def showEvent(self,e):
        super().showEvent(e); self.search_input.setFocus(); self.search_input.selectAll(); self._converted_book=False; self._converted_book_name=""; self._selected_book=None; self._stage="book"; self._space_mode=False; self.result_list.clear()
    @staticmethod
    ...
    def _set_text(self,t,converted=None):
        ...
        try:self.search_input.setText(str(t or "")); self.search_input.setCursorPosition(len(str(t or "")))
        finally:self._formatting=False
        if converted is not None:self._converted_book=bool(converted); self._converted_book_name=str(t or "") if converted else ""
    ...
    def _select_current_book(self):
        ...
        self._selected_book=b; self._stage="chapter"; self._space_mode=False; self._set_text(b,True); self.result_list.clear(); self.search_input.setFocus(); self.search_input.setCursorPosition(len(b)); self.hint_label.setText(f"已选择 {b}　输入章节后按空格进入节号"); return True
    ...
    def _space_after_chapter(self):
        ...
        if not 1<=c<=mx:self.hint_label.setText(f"章节超出范围，本书最多 {mx} 章"); return True
        self._stage="verse"; self._set_text(f"{self._selected_book} {c}:"); self.hint_label.setText("请输入开始节，按空格自动生成 -"); return True
    def _space_after_verse(self):
        ...
        if not 1<=v<=mx:self.hint_label.setText(f"本章最多 {mx} 节"); return True
        self._space_mode=True; self._set_text(f"{self._selected_book} {c}:{v}-"); self.hint_label.setText(f"请输入结束节（{v}-{mx}）"); return True
    def _delete_segment(self):
        ...
        # 严格按“结束节→范围符→开始节→章分隔符→章→书卷”逐级删除。
        if re.fullmatch(r"\s*\d+\s*:\s*\d+\s*-\s*\d+\s*",suffix):
            s=suffix.strip(); c,v,e=re.fullmatch(r"(\d+)\s*:\s*(\d+)\s*-\s*(\d+)",s).groups(); self._set_text(f"{b} {c}:{v}-"); self._stage="verse"; self._space_mode=True; return True
        if re.fullmatch(r"\s*\d+\s*:\s*\d+\s*-\s*",suffix):
            s=suffix.strip(); c,v=re.fullmatch(r"(\d+)\s*:\s*(\d+)\s*-",s).groups(); self._set_text(f"{b} {c}:{v}"); self._stage="verse"; self._space_mode=False; return True
        ...
        if re.fullmatch(r"\s*\d+\s*",suffix):
            self._set_text(b,True); self._stage="chapter"; self._space_mode=False; self.result_list.clear(); return True
        self._set_text(""); self._selected_book=None; self._stage="book"; self._space_mode=False; self.result_list.clear(); return True
    ...
    def _refresh_selected(self,b,suffix):
        ...
        if not 1<=c<=mx:self.hint_label.setText(f"章节超出范围，本书最多 {mx} 章");return
        if v is None:
            item=QListWidgetItem(f"▶ {b} {c}章（整章）");item.setData(Qt.ItemDataRole.UserRole,(b,c,None,None));self.result_list.addItem(item);self.result_list.setCurrentRow(0);return
        v=int(v); mv=self._verse_count(b,c)
        if not 1<=v<=mv:self.hint_label.setText(f"第 {c} 章最多 {mv} 节");return
        if e is None:
            if "-" in s:self.hint_label.setText(f"请输入结束节（{v}-{mv}）");return
            item=QListWidgetItem(f"▶ {b} {c}:{v}");item.setData(Qt.ItemDataRole.UserRole,(b,c,v,v));self.result_list.addItem(item);self.result_list.setCurrentRow(0);return
        if e=="":self.hint_label.setText(f"请输入结束节（{v}-{mv}）");return
        e=int(e)
        if e<v:self.hint_label.setText(f"结束节不能小于开始节 {v}");return
        if e>mv:self.hint_label.setText(f"本章最多 {mv} 节，不能输入 {e}");return
        item=QListWidgetItem(f"▶ {b} {c}:{v}-{e}");item.setData(Qt.ItemDataRole.UserRole,(b,c,v,e));self.result_list.addItem(item);self.result_list.setCurrentRow(0)
    ...
    def _on_item_clicked(self,item):
        ...
        b=d[0] if isinstance(d,tuple) else d; self._selected_book=b; self._stage="chapter"; self._set_text(b,True); self.result_list.clear(); self.search_input.setFocus()
