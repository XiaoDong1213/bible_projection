# ui/toolbar.py
# 顶部工具栏：保留扩展/滚动/主题，将所有显示设置统一收纳到“显示设置”面板
from PyQt6.QtWidgets import (
    QToolBar, QPushButton, QLabel, QSlider, QDialog, QFormLayout,
    QGroupBox, QHBoxLayout, QVBoxLayout, QSpinBox, QFontComboBox,
    QColorDialog, QFileDialog, QLineEdit, QDialogButtonBox
)
...
class DisplaySettingsDialog(QDialog):
    ...
    def _build_ui(self):
        ...
        form.addRow("正文", self.font_combo)
        form.addRow("正文字号", self.font_size)
        form.addRow("正文字色", self.font_color_btn)
        form.addRow("标题字体", self.title_font_combo)
        form.addRow("标题字号", self.title_size)
        form.addRow("标题颜色", self.title_color_btn)
        form.addRow("节号字体", self.verse_font_combo)
        form.addRow("节号字号", self.verse_size)
        form.addRow("节号颜色", self.verse_color_btn)
        form.addRow("底注字体", self.footer_font_combo)
        form.addRow("底注字号", self.footer_size)
        form.addRow("底注颜色", self.footer_color_btn)
        ...
        self.font_color_btn.clicked.connect(lambda: self._choose_color("font_color"))
        self.title_color_btn.clicked.connect(lambda: self._choose_color("title_color"))
        self.verse_color_btn.clicked.connect(lambda: self._choose_color("verse_num_color"))
        self.footer_color_btn.clicked.connect(lambda: self._choose_color("footer_color"))
    ...
    def _set_color_button(self, button, color):
        ...
        button.setStyleSheet(
            f"background:{color.name()};color:{text};padding:6px 12px;border-radius:4px;"
        )
    ...
    def _choose_color(self, key):
        ...
        if color.isValid():
            ...
            button = {
                "font_color": self.font_color_btn,
                "title_color": self.title_color_btn,
                "verse_num_color": self.verse_color_btn,
                "footer_color": self.footer_color_btn,
    ...
    def _load_settings(self):
        ...
        for key, button in (
            ...
            self._set_color_button(button, s.get(key, "#FFFFFF"))
...
class ToolBarWidget(QToolBar):
    ...
    def __init__(self, parent=None):
        ...
        self.scroll_slider.setFixedWidth(120)
    ...
    def _on_scroll_changed(self, speed):
        names = ["暂停", "1档", "2档", "3档", "4档", "5档", "6档"]
        self.speed_label.setText(names[speed])
