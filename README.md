...
Bible Pro 提供 **经文全文搜索** 侧栏（可由功能开关启用），挂在主窗口右侧并随窗口缩放。

- 两种匹配：**相近**（按字顺序，中间可少打）与 **一致**（须连续出现）。
- 空格分隔多个词时，同一节须全部满足（例如：`爱 盼望`）。
- 支持按书卷选择搜索范围（旧约 / 新约 / 指定书卷）。
- 搜索结果分页显示书卷、章节、节号与经文内容。
- 可从结果定位到对应经文，或直接投影。
- 保留最近搜索历史，支持单条删除与清空。
- `Ctrl + F` 打开或关闭经文全文搜索。
