...
class AppConfig:
    ...
    def save_scripture_search_history(self, history_list):
        """保存经文全文搜索历史。"""
        if "ScriptureSearch" not in self.parser:
            self.parser["ScriptureSearch"] = {"history": "[]"}
        self.parser["ScriptureSearch"]["history"] = json.dumps(history_list, ensure_ascii=False)
        self._save_ini()

    def load_scripture_search_history(self):
        """读取经文全文搜索历史。"""
        if "ScriptureSearch" not in self.parser:
            return []
        raw = self.parser["ScriptureSearch"].get("history", "[]")
        try:
